...
public class Transicao {
  ...
  private String estadoAtual,palavraLida;
    ...
    */
    try{
      if(! verificarEntrada()){
        ...
        palavraLida = palavraEntradaAux.get(cabecaFita + 1); // Armazena a primeiro símbolo a ser lida
        ...
        boolean aux = true; // Variável auxiliar para reconfiguração da cabeça da fita para posição inicial
          while(cabecaFita != 0){ // Laço de repetição com parada quando a cabeça da fita voltar à posição inicial
             ...
             */
            if(estadoAtual.equals(retornarEstado(maquinaAux.getTransicoes(), posicaoAtual)) && palavraLida.equals(maquinaAux.getTransicoes().get(posicaoAtual + 1))){
              ...
              if(cabecaFita < palavraEntradaAux.size() -1){ // Verifica se chegou no fim da palavra de entrada
                ...
                palavraLida = palavraEntradaAux.get(cabecaFita + 1); // Armazena a próximo símbolo a ser lida
